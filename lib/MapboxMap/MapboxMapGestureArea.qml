...
Item {
    ...
    PinchArea {
        ...
        //! Holds map bearing at the start of the current pinch
        property double __oldBearing

        //! Pivot geographic coordinate (point under pinch.center at gesture start)
        property double __pivotLat: 0
        property double __pivotLon: 0
        property double __pivotScreenX: 0
        property double __pivotScreenY: 0

        //! After setting map.bearing, shift map.center so that (__pivotLat,__pivotLon)
        //! remains at (__pivotScreenX,__pivotScreenY) on screen.
        function correctCenterForBearing(newBearing) {
            var dx  = __pivotScreenX - map.width  / 2
            var dy  = __pivotScreenY - map.height / 2
            var br  = newBearing * Math.PI / 180
            var mpp = map.metersPerPixel
            var dE  =  (dx * Math.cos(br) - dy * Math.sin(br)) * mpp
            var dN  = (-dx * Math.sin(br) - dy * Math.cos(br)) * mpp
            var lat = __pivotLat - dN / 111319.49
            var lon = __pivotLon - dE / (111319.49 * Math.cos(__pivotLat * Math.PI / 180))
        ...
        //! Save previous zoom level and bearing when pinch gesture started
        onPinchStarted: {
            pinchAnim.stop()
            __oldZoom    = map.zoomLevel;
            __oldBearing = map.bearing;
            __startTime  = new Date().getTime();
            __rate = 0; __reference = __oldZoom;
            __lastUpdateValue = __oldZoom; __lastUpdateTime = __startTime;

            // Record geographic coordinate under pinch.center
            var dx  = pinch.center.x - map.width  / 2
            var dy  = pinch.center.y - map.height / 2
            var br  = map.bearing * Math.PI / 180
            var mpp = map.metersPerPixel
            var dE  =  (dx * Math.cos(br) - dy * Math.sin(br)) * mpp
            var dN  = (-dx * Math.sin(br) - dy * Math.cos(br)) * mpp
            __pivotLat     = map.center.latitude  + dN / 111319.49
            __pivotLon     = map.center.longitude + dE / (111319.49 * Math.cos(map.center.latitude * Math.PI / 180))
            __pivotScreenX = pinch.center.x
            __pivotScreenY = pinch.center.y
        }

        //! Update map's zoom level and bearing while pinching
        onPinchUpdated: {
            var newZoom    = calcZoomDelta(__oldZoom, pinch.scale)
            var newBearing = __oldBearing - pinch.rotation
            map.setZoomLevel(newZoom, pinch.center)
            map.bearing = newBearing
            correctCenterForBearing(newBearing)
            calcRate(newZoom);
            __lastUpdateValue = newZoom;
            __lastUpdateTime = new Date().getTime();
        }

        //! Finalise zoom level and bearing when pinch ends
        onPinchFinished: {
            var newZoom    = calcZoomDelta(__oldZoom, pinch.scale)
            var newBearing = __oldBearing - pinch.rotation
