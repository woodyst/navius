...
// ── Preferences panel overlay ──────────────────────────────────────────
    ...
    RouteSelectPanel {
        ...
        onNavigationRequested: function(idx) {
            ...
            // Con "Precargar mapa" apagado —que es lo normal— se arranca sin
            // bajar nada: precargar cuesta medio minuto de espera y unos megas,
            // y solo compensa yendo a una zona sin cobertura. Con mapas locales
            // tampoco hay nada que bajar.
            if (!mapView._usingOsmScoutMaps && searchPanel.preCacheTiles
                    && rd && rd.shape && rd.shape.length > 1) {
                mapView.followMode = false
